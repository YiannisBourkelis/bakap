...
# Bakap fail2ban filter for Samba - AUTOMATICALLY CONFIGURED
# This file is automatically generated by bakap setup.sh
...
# Example: check_ntlm_password: Authentication for user [username] -> [username] FAILED with error NT_STATUS_NO_SUCH_USER
failregex = ^.*check_ntlm_password:.*Authentication for user .* FAILED with error .*
            ^.*Auth:.*user .* at .* remote host \[ipv4:<HOST>:\d+\].*status \[NT_STATUS_
